...
export const useFlowbiteThemable = (_theme?: FlowbiteTheme): UseFlowbiteThemableReturns => {
  const theme = inject<Ref<FlowbiteTheme | null>>(FLOWBITE_THEMABLE_INJECTION_KEY, ref(null));

  const isActive: ComputedRef<boolean> = computed(() => !!theme?.value);
  const color: ComputedRef<FlowbiteTheme | undefined> = computed(() => theme?.value || undefined);
  ...
  const hoverClasses: ComputedRef<string> = computed(() => {
    if (!themeName.value) return '';
  ...
  const borderClasses: ComputedRef<string> = computed(() => {
    if (!themeName.value) return '';
    return flowbiteThemeClasses[themeName.value].border;
  });

  const focusClasses: ComputedRef<string> = computed(() => {
    if (!themeName.value) return '';
