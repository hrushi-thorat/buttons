...
const buttonColorClasses: ButtonClassMap<ButtonVariant> = {
  default: {
    ...
    // Custom
    primary: '',
    secondary: 'text-white bg-secondary-500 focus:outline-none focus:ring-2 focus:ring-secondary-600',
    tertiary: 'text-white bg-tertiary-500 focus:outline-none focus:ring-2 focus:ring-tertiary-200',
    success: 'bg-white text-black focus:outline-none focus:ring-2 focus:ring-tertiary-500',
    link: 'text-gray-700 dark:text-white bg-white dark:bg-gray-900 focus:outline-none',
    gray: 'bg-gray-600 text-white focus:outline-none focus:bg-gray-700',
  },
  hover: {
    default: 'hover:bg-primary-400 dark:hover:bg-blue-700',
    ...
    tertiary: 'hover:bg-tertiary-400',
    success: '',
...
const buttonOutlineColorClasses: ButtonClassMap<Exclude<ButtonVariant, 'light' | 'alternative'>> = {
  default: {
    ...
    default:
      'text-primary-500 border border-primary-500 focus:ring-4 focus:outline-none focus:ring-primary-300 text-center dark:border-blue-500 dark:text-blue-500 dark:focus:ring-blue-800',
    ...
    tertiary: 'text-tertiary-500 bg-transparent border border-tertiary-500 focus:outline-none focus:ring-2 focus:ring-tertiary-200',
    success:
    ...
    gray: 'text-gray-800 border border-gray-800 focus:outline-none',
  },
  hover: {
    dark: 'hover:text-white hover:bg-gray-600',
    default: 'hover:text-primary-500 hover:border hover:border-primary-400 dark:hover:text-white dark:hover:bg-blue-600',
...
const simpleGradients: string[] = ['blue', 'green', 'cyan', 'teal', 'lime', 'red', 'pink', 'purple'];
const alternativeColors: string[] = ['alternative', 'light'];

export const useButtonClasses = (
  props: UseButtonClassesProps
): {
  ...
  const bindClasses = computed(() => {
    const isGradient: boolean = !!props.gradient.value;
    const isColor: boolean = !!props.color.value;
    const isOutline: boolean = props.outline.value;
    ...
    if (isGradient && isOutline) {
      ...
      if (props.gradient && !simpleGradients.includes(props.gradient.value!)) {
        backgroundClass =
        ...
        console.warn(`cannot use outline prop with "${props.gradient && props.gradient.value}" gradient`); // TODO: prettify
      }
    } else if (isGradient) {
      // JUST GRADIENT
      ...
      if (!alternativeColors.includes(props.color.value)) {
        const color: string = props.color.value;
  ...
  const spanClasses: ComputedRef<string> = computed(() => {
    ...
    //   );
    // }
    return 'text-sm md:text-md flex items-center justify-center h-full';
  });
